...
def build_multi_crypto_dataset(fetch_fn, days=90, window=10, steps_ahead=5):
    X_all, Y_all = [], []
    for name, gecko_id in cryptos.items():
        ...
        X, Y = build_sequences(prices_norm, window, steps_ahead=steps_ahead)
        X_all.append(X)
        Y_all.append(Y)

    X_total = np.vstack(X_all)
    Y_total = np.concatenate(Y_all)
    return X_total, Y_total, mean, std, prices_norm
...
def train_lstm(window=10, steps_ahead=50, epochs=500, lr=0.01):
    #prices = np.array([float(e[1]) for e in data['prices']], dtype=np.float32)
    from coingeckoAPI import fetch_token_price
    X, Y,mean, std,returns_norm = build_multi_crypto_dataset(fetch_fn=fetch_token_price,days=180,window=window,steps_ahead=steps_ahead)#build_sequences(returns_norm, window, steps_ahead)
...
def train():
    ...
    predicted_prices, model , prices, epoch_graph = train_lstm(lr=0.001, window=15, epochs=500000)
